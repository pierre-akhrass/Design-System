...
interface CheckboxControlsProps {
  ...
  onChange: (cfg: CheckboxConfig) => void
}
...
export const CheckboxControls = ({ config, onChange }: CheckboxControlsProps) => {
  ...
  return (
    <div className="checkbox-controls">
      ...
      <ControlRow label="state">
        <Segmented options={['unchecked', 'checked', 'indeterminate']} value={config.state} onChange={(v) => set('state', v as CheckboxConfig['state'])} />
      </ControlRow>

      <ControlRow label="placement">
        <Segmented options={['left', 'right']} value={config.placement} onChange={(v) => set('placement', v as CheckboxConfig['placement'])} />
      </ControlRow>
      ...
      <ControlRow label="className">
        <input className="ctrl-input ctrl-input--full" type="text" value={config.customClass} onChange={(e) => set('customClass', e.target.value)} placeholder="e.g. my-checkbox" spellCheck={false} />
      </ControlRow>

      <ControlRow label="id">
        <input className="ctrl-input ctrl-input--full" type="text" value={config.customId} onChange={(e) => set('customId', e.target.value)} placeholder="e.g. terms-checkbox" spellCheck={false} />
      </ControlRow>

      <div className="ctrl-css-wrap">
        <textarea className="ctrl-css-textarea" value={config.customCss} onChange={(e) => set('customCss', e.target.value)} placeholder={`.my-checkbox { ... }`} spellCheck={false} rows={5} />
      </div>
    </div>
  )
}
