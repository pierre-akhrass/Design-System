import type { MapConfig, MapPinConfig } from './mapCodeGen'
...
export const MapControls = ({ config, onChange }: MapControlsProps) => {
  ...
  return (
    <div className="map-controls">
      ...
      <ControlRow label="title">
        <input className="ctrl-input ctrl-input--full" type="text" value={config.title} onChange={(e) => set('title', e.target.value)} placeholder="Map title" spellCheck={false} />
      </ControlRow>

      <ControlRow label="description">
        <input className="ctrl-input ctrl-input--full" type="text" value={config.description} onChange={(e) => set('description', e.target.value)} placeholder="Description text" spellCheck={false} />
      </ControlRow>

      <ControlRow label="mapImage">
        <input className="ctrl-input ctrl-input--full" type="text" value={config.mapImage} onChange={(e) => set('mapImage', e.target.value)} placeholder="URL to background map image" spellCheck={false} />
      </ControlRow>

      <Divider />
      <SectionLabel>Pins ({config.pins.length})</SectionLabel>

      <div className="map-controls__pins">
        {config.pins.map((pin, i) => (
          <div key={pin.id} className="map-controls__pin-card">
            <div className="map-controls__pin-header">
              <span className="map-controls__pin-index">#{i + 1}</span>
              <button
                className="map-controls__pin-remove"
                type="button"
                onClick={() => set('pins', config.pins.filter((_, idx) => idx !== i))}
                aria-label={`Remove pin ${pin.label}`}
              >✕</button>
            </div>
            <div className="map-controls__pin-fields">
              <input className="ctrl-input ctrl-input--full" type="text" value={pin.label} placeholder="Label" spellCheck={false}
                onChange={(e) => { const pins = [...config.pins]; pins[i] = { ...pins[i], label: e.target.value }; set('pins', pins) }} />
              <div className="map-controls__pin-row">
                <input className="ctrl-input" type="number" value={pin.x} min={0} max={100} style={{ width: 60 }}
                  onChange={(e) => { const pins = [...config.pins]; pins[i] = { ...pins[i], x: Number(e.target.value) }; set('pins', pins) }} />
                <span className="map-controls__pin-sep">x</span>
                <input className="ctrl-input" type="number" value={pin.y} min={0} max={100} style={{ width: 60 }}
                  onChange={(e) => { const pins = [...config.pins]; pins[i] = { ...pins[i], y: Number(e.target.value) }; set('pins', pins) }} />
                <span className="map-controls__pin-sep">%</span>
              </div>
              <input className="ctrl-input ctrl-input--full" type="text" value={pin.description} placeholder="Description" spellCheck={false}
                onChange={(e) => { const pins = [...config.pins]; pins[i] = { ...pins[i], description: e.target.value }; set('pins', pins) }} />
              <div className="map-controls__pin-row">
                <input className="ctrl-input" type="text" value={pin.country} placeholder="country" spellCheck={false} style={{ flex: 1 }}
                  onChange={(e) => { const pins = [...config.pins]; pins[i] = { ...pins[i], country: e.target.value }; set('pins', pins) }} />
                <input className="ctrl-input" type="text" value={pin.project} placeholder="project" spellCheck={false} style={{ flex: 1 }}
                  onChange={(e) => { const pins = [...config.pins]; pins[i] = { ...pins[i], project: e.target.value }; set('pins', pins) }} />
              </div>
            </div>
          </div>
        ))}
        <div className="map-controls__pin-actions">
          <button className="map-controls__pin-add" type="button" onClick={() => {
            const id = String(Date.now())
            const newPin: MapPinConfig = { id, x: 50, y: 50, label: `Pin ${config.pins.length + 1}`, description: '', country: '', project: '' }
            set('pins', [...config.pins, newPin])
          }}>+ Add pin</button>
          <button className="map-controls__pin-reset" type="button" onClick={() => set('pins', DEFAULT_PINS)}>Reset</button>
        </div>
      </div>
      ...
      <ControlRow label="mode">
        <Segmented options={['light', 'dark']} value={config.mode} onChange={(v) => set('mode', v as MapConfig['mode'])} />
      </ControlRow>

      <ControlRow label="showFilters">
        <Toggle value={config.showFilters} onChange={(v) => set('showFilters', v)} label="Toggle filters bar" />
      </ControlRow>
      ...
      <ControlRow label="className">
        <input className="ctrl-input ctrl-input--full" type="text" value={config.customClass} onChange={(e) => set('customClass', e.target.value)} placeholder="e.g. my-map" spellCheck={false} />
      </ControlRow>

      <ControlRow label="id">
        <input className="ctrl-input ctrl-input--full" type="text" value={config.customId} onChange={(e) => set('customId', e.target.value)} placeholder="e.g. hero-map" spellCheck={false} />
      </ControlRow>

      <div className="ctrl-css-wrap">
        <textarea className="ctrl-css-textarea" value={config.customCss} onChange={(e) => set('customCss', e.target.value)} placeholder={`.my-map { ... }`} spellCheck={false} rows={5} />
      </div>
    </div>
  )
}
