...
export const RadioControls = ({ config, onChange }: RadioControlsProps) => {
  ...
  return (
    <div className="radio-controls">
      ...
      <ControlRow label="state">
        <Segmented options={['unchecked', 'checked']} value={config.state} onChange={(v) => set('state', v as RadioConfig['state'])} />
      </ControlRow>

      <ControlRow label="placement">
        <Segmented options={['left', 'right']} value={config.placement} onChange={(v) => set('placement', v as RadioConfig['placement'])} />
      </ControlRow>
      ...
      <ControlRow label="className">
        <input className="ctrl-input ctrl-input--full" type="text" value={config.customClass} onChange={(e) => set('customClass', e.target.value)} placeholder="e.g. my-radio" spellCheck={false} />
      </ControlRow>

      <ControlRow label="id">
        <input className="ctrl-input ctrl-input--full" type="text" value={config.customId} onChange={(e) => set('customId', e.target.value)} placeholder="e.g. option-radio" spellCheck={false} />
      </ControlRow>

      <div className="ctrl-css-wrap">
        <textarea className="ctrl-css-textarea" value={config.customCss} onChange={(e) => set('customCss', e.target.value)} placeholder={`.my-radio { ... }`} spellCheck={false} rows={5} />
      </div>
    </div>
  )
}
