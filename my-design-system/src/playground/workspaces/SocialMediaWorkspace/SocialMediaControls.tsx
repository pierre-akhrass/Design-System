...
interface SocialMediaControlsProps {
  ...
  onChange: (cfg: SocialMediaConfig) => void
}
...
// $radius-* — media default = $radius-medium = 8px
const RADIUS_OPTIONS: TokenOption[] = [
  { value: '', label: 'token', sub: n(TOKENS.radius.md) + 'px' },
...
// $padding-action-* — text-card padding default = $padding-action-s = 16px
...
export const SocialMediaControls = ({ config, onChange }: SocialMediaControlsProps) => {
  const set = <K extends keyof SocialMediaConfig>(key: K, value: SocialMediaConfig[K]) => onChange({ ...config, [key]: value })

  return (
    <div className="social-media-controls">
      <SectionLabel>Content</SectionLabel>

      <ControlRow label="platform" badge="required" stack>
        <Segmented options={['facebook', 'instagram', 'tiktok', 'youtube', 'x']} value={config.platform} onChange={(v) => set('platform', v as SocialMediaConfig['platform'])} />
      </ControlRow>

      <ControlRow label="type">
        <Segmented options={['image', 'video', 'text']} value={config.type} onChange={(v) => set('type', v as SocialMediaConfig['type'])} />
      </ControlRow>

      {config.type === 'image' && (
        <>
          <ControlRow label="caption">
            <input className="ctrl-input ctrl-input--full" type="text" value={config.caption} onChange={(e) => set('caption', e.target.value)} placeholder="Caption text" spellCheck={false} />
          </ControlRow>
          <ControlRow label="pagination">
            <Toggle value={config.showPagination} onChange={(v) => set('showPagination', v)} label="Toggle pagination" />
          </ControlRow>
        </>
      )}
      ...
      {config.type === 'text' && (
        <>
          <ControlRow label="text" stack>
            <textarea className="ctrl-css-textarea" style={{ minHeight: 54 }} value={config.text} onChange={(e) => set('text', e.target.value)} placeholder="Post text" spellCheck={false} rows={3} />
          </ControlRow>
          <ControlRow label="hashtags">
            <input className="ctrl-input ctrl-input--full" type="text" value={config.hashtags} onChange={(e) => set('hashtags', e.target.value)} placeholder="#tag #tag" spellCheck={false} />
          </ControlRow>
        </>
      )}
      ...
      <ControlRow label="theme">
        <Segmented options={['light', 'dark']} value={config.theme} onChange={(v) => set('theme', v as SocialMediaConfig['theme'])} />
      </ControlRow>

      <Divider />
      <SectionLabel>Color overrides</SectionLabel>

      <ControlRow label="cardBg">
        <ColorRow value={config.cardBg} defaultSwatch="#eef1f4" onChange={(v) => set('cardBg', v)} onClear={() => set('cardBg', '')} />
      </ControlRow>
      ...
      <ControlRow label="radius" stack>
        <TokenField options={RADIUS_OPTIONS} value={config.radius} onChange={(v) => set('radius', v)} />
      </ControlRow>

      <ControlRow label="text padding" stack disabled={config.type !== 'text'}>
        <TokenField options={PADDING_OPTIONS} value={config.textPadding} onChange={(v) => set('textPadding', v)} />
      </ControlRow>
      ...
      <ControlRow label="className">
        <input className="ctrl-input ctrl-input--full" type="text" value={config.customClass} onChange={(e) => set('customClass', e.target.value)} placeholder="e.g. my-post" spellCheck={false} />
      </ControlRow>

      <ControlRow label="id">
        <input className="ctrl-input ctrl-input--full" type="text" value={config.customId} onChange={(e) => set('customId', e.target.value)} placeholder="e.g. feed-post" spellCheck={false} />
      </ControlRow>

      <div className="ctrl-css-wrap">
        <textarea className="ctrl-css-textarea" value={config.customCss} onChange={(e) => set('customCss', e.target.value)} placeholder={`.ds-social-media-post__media { box-shadow: 0 6px 16px rgba(0,0,0,.12); }`} spellCheck={false} rows={5} />
        {config.customCss && (
