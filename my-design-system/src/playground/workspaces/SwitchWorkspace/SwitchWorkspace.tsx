...
export const SwitchWorkspace = () => {
  ...
  useScssSync<SwitchConfig>('switch', setConfig)
  ...
  useEffect(() => {
    ...
    if (config.fontSize) rules.push(`.ds-switch { font-size: ${config.fontSize} !important; }`)
    if (config.fontWeight) rules.push(`.ds-switch { font-weight: ${config.fontWeight} !important; }`)
    if (config.letterSpacing) rules.push(`.ds-switch { letter-spacing: ${config.letterSpacing} !important; }`)
    if (config.textTransform && config.textTransform !== 'none') rules.push(`.ds-switch { text-transform: ${config.textTransform} !important; }`)
    if (config.shadow) rules.push(`.ds-switch { box-shadow: ${config.shadow} !important; }`)
