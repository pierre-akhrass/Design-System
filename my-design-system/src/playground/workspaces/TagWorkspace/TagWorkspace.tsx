...
export const TagWorkspace = () => {
  ...
  useEffect(() => {
    const rules: string[] = []
    if (config.fontFamily) rules.push(`.ds-tag { font-family: ${config.fontFamily} !important; }`)
    if (config.fontSize) rules.push(`.ds-tag { font-size: ${config.fontSize} !important; }`)
    if (config.fontWeight) rules.push(`.ds-tag { font-weight: ${config.fontWeight} !important; }`)
    if (config.letterSpacing) rules.push(`.ds-tag { letter-spacing: ${config.letterSpacing} !important; }`)
    if (config.textTransform && config.textTransform !== 'none') rules.push(`.ds-tag { text-transform: ${config.textTransform} !important; }`)
    if (config.shadow) rules.push(`.ds-tag { box-shadow: ${config.shadow} !important; }`)
