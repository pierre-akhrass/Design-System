...
import './TextAreaControls.scss'
import { TypographyControls } from '../../TypographyControls'

interface TextAreaControlsProps {
  config: TextAreaConfig
  onChange: (cfg: TextAreaConfig) => void
}
...
export const TextAreaControls = ({ config, onChange }: TextAreaControlsProps) => {
  ...
  return (
    <div className="textarea-controls">
      ...
      <ControlRow label="value">
        <input className="ctrl-input ctrl-input--full" type="text" value={config.value} onChange={(e) => set('value', e.target.value)} placeholder="Value text" spellCheck={false} />
      </ControlRow>

      <Divider />
      <SectionLabel>Behaviour</SectionLabel>

      <ControlRow label="valueType">
        <Segmented options={['default', 'placeholder']} value={config.valueType} onChange={(v) => set('valueType', v as TextAreaConfig['valueType'])} />
      </ControlRow>

      <ControlRow label="state">
        <Segmented options={['default', 'placeholder', 'focused', 'disabled']} value={config.state} onChange={(v) => set('state', v as TextAreaConfig['state'])} />
      </ControlRow>
      ...
      <ControlRow label="className">
        <input className="ctrl-input ctrl-input--full" type="text" value={config.customClass} onChange={(e) => set('customClass', e.target.value)} placeholder="e.g. my-textarea" spellCheck={false} />
      </ControlRow>

      <ControlRow label="id">
        <input className="ctrl-input ctrl-input--full" type="text" value={config.customId} onChange={(e) => set('customId', e.target.value)} placeholder="e.g. bio-textarea" spellCheck={false} />
      </ControlRow>

      <div className="ctrl-css-wrap">
        <textarea className="ctrl-css-textarea" value={config.customCss} onChange={(e) => set('customCss', e.target.value)} placeholder={`.my-textarea { ... }`} spellCheck={false} rows={5} />
      </div>
    </div>
  )
}
